...
def model_state_update(model, t, state_controller, input_f16):
    ...
    if state == GcasAutopilot.STATE_START:
        if t >= man_start: state = GcasAutopilot.STATE_ROLL
        ...
        radsFromNoseLevel = round(gamma/(2 * np.pi))

        if gamma - 2 * np.pi * radsFromNoseLevel > path_goal:
            state = GcasAutopilot.STATE_DONE

    if(old_state != state): logger.info(f'State Tr: {old_state} -> {state}')
...
def model_output(model, t, state_controller, input_f16):
    ...
    #TODO: Nz_des should be nested under cntrlLmits?
    ...
    #import sympy as sym
    #phi, p, theta, alpha = sym.var('phi, p, theta, alpha')
    ...
    if state == GcasAutopilot.STATE_START:
        ...
        Nz, ps = 0, state_roll(phi_des, phi, p_des, p)
    elif state == GcasAutopilot.STATE_PULL:
        Nz, ps = state_pull(Nz_des), 0
    elif state == GcasAutopilot.STATE_DONE:
        Nz, ps = state_done(gamma_des, phi_des, p_des, gamma, phi, p, q)
...
def state_roll(phi_des, phi, p_des, p):
    # Determine which angle is "level" (0, 180, 360, 720, etc)
    ...
    #ps = -(phi - phi_des) * K_prop + (p_des - p) * K_der
    ps = ah.pd_cntrl(K_prop, K_der, phi_des-phi, p_des-p)
    return ps
...
def state_done(gamma_des, phi_des, p_des, gamma, phi, p, q):
    # steady-level hold
    ...
    e_nz, ed_nz = gamma_des-gamma, 0-q
    #hdot = vt*sin(gamma)
