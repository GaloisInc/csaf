...
def model_init(model):
    path = "/home/greg/Documents/csaf_architecture/src/spinningup/data/env_fn()/env_fn()_s0/tf1_backup"
...
def model_output(model, time_t, state_controller, input_f16):
    ...
    state[1] = input_f16[1] / 45
    state[2] = input_f16[2] / 30
    state[3] = input_f16[3] / 180
    state[4] = input_f16[4] / 180
    state[5] = input_f16[5] / 180
    state[6] = input_f16[6] / 180
    state[7] = input_f16[7] / 180
    state[8] = input_f16[8] / 180
    ...
    # Unnormalize action
    return action[0] * 9, action[1] * 3, 0, action[2]
...
def model_state_update(model, time_t, state_controller, input_f16):
    ...
    if state == NNAutopilot.STATE_START:
        if time_t >= 2:
            state = NNAutopilot.STATE_ENGAGED

    elif state == NNAutopilot.STATE_ENGAGED:
        if h >= 800 and theta >= 0 and theta <= 30:
