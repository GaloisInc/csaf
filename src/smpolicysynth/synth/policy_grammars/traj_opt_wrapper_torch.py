...
class TrapOptWrapper:
	...
	def get_cost(self, x, with_grad = False, vis = False):
		#print(self.counter)
		self.counter += 1
		full_x = np.copy(self.full_x)
		assert(len(self.sub_indices) == len(x))
		...
		full_x = torch.tensor(full_x, requires_grad = with_grad)
		...
		for i in range(self.num_init_states):
			...
			safe_err, goal_err, mode_err, cond_err, total_time, total_obj = policies[i].evaluate(init_state, with_grad, vis)
			...
			if self.err_mode == ErrMode.Safe:
				...
				e += w1*safe_err + np.sum(goal_err) + w3*mode_err + w4*cond_err
		...
		if with_grad:
			err.backward()
			#print(full_x)
			grad = full_x.grad.numpy()
			#print("grad", full_x.grad)
