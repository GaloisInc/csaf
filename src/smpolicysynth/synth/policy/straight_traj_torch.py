...
class StraightTraj:
	...
	def __init__(self, env, modes, max_timesteps, dts, ref_trajs, ref_prob):
		self.modes = modes
	...
	def get_traj_from_k_mode_policy(self, env, init_state, with_grad = False, vis = False):
		states = []
		...
		if len(self.ref_trajs) > 0:
			...
			for i in range(len(ref_modes)):
				...
				weight += self.ref_prob[i] * torch.exp(-dist/1.0)
				count += self.ref_prob[i]
			weight = weight/count
			mode_err += -torch.log(weight)
	...
	def evaluate(self, init_state, with_grad = False, vis = False, id = 0):
		states, safe_err, goal_err, mode_err, cond_err, total_time, total_obj, mode_change_states = self.get_traj_from_k_mode_policy(self.env, init_state, with_grad, vis)
