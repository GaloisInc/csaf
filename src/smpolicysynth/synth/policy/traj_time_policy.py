...
class TrajTimePolicy:
	...
	def get_action(self, state):
		...
		if self.cur_time < time_for_mode:
			dt = self.get_dt(self.cur_time, time_for_mode)
		else:
			self.cur_id += 1
			self.cur_time = 0.0
			if (self.cur_id >= len(self.modes)):
				...
				time_for_next_mode = self.times[self.cur_id]
				dt = self.get_dt(self.cur_time, time_for_next_mode)
	...
	def evaluate(self, init_state, vis = False, id = -1):
